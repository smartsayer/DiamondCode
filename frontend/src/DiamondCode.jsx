...
function ParlayCalculator() {
  ...
  const oddsToDecimal = (american) => {
    if (american >= 0) return 1 + american / 100;
    return 1 + 100 / Math.abs(american);
  };

  const oddsToProb = (american) => {
  ...
  const validLegs = legs.filter(l => parseOdds(l.odds) !== null);
  const wagerAmt = parseFloat(wager) || 0;

  const combinedDecimal = validLegs.reduce((acc, l) => acc * oddsToDecimal(parseOdds(l.odds)), 1);
  const combinedProb = validLegs.reduce((acc, l) => acc * oddsToProb(parseOdds(l.odds)), 1);
  const payout = wagerAmt * combinedDecimal;
  const profit = payout - wagerAmt;
  const combinedOdds = validLegs.length > 0 ? formatAmerican(combinedDecimal) : "—";

  const addLeg = () => {
    if (legs.length >= 12) return;
    setLegs(prev => [...prev, { id: nextId, label: "", odds: "" }]);
    setNextId(n => n + 1);
  };
  ...
  const clearAll = () => {
    setLegs([{ id: 1, label: "", odds: "" }, { id: 2, label: "", odds: "" }]);
    setNextId(3);
    setWager("100");
  };

  const oddsColor = (american) => {
    if (american === null) return "#333";
    if (american >= 100) return "#a78bfa";
    if (american <= -200) return "#fb7185";
    return "#fbbf24";
  };

  const tierColor = validLegs.length < 2 ? "#333"
    : combinedProb < 0.05 ? "#a78bfa"
    : combinedProb < 0.15 ? "#00ff87"
    : combinedProb < 0.30 ? "#fbbf24"
    : "#fb7185";

  const tierLabel = validLegs.length < 2 ? "ADD LEGS"
    : combinedProb < 0.05 ? "LOTTERY TICKET"
    : combinedProb < 0.15 ? "HIGH RISK"
    : combinedProb < 0.30 ? "MODERATE"
    : "SAFER PLAY";
  ...
  return (
    <div style={{ padding: "4px 0" }}>
      {/* Header */}
      <div style={{
        background: "linear-gradient(135deg, #0a0a0a 0%, #001a0d 100%)",
        border: "2px solid #00ff8730", borderRadius: 10,
        padding: "18px 20px", marginBottom: 16,
      }}>
        <div style={{ fontSize: 11, color: "#00ff87", letterSpacing: 3, fontWeight: 900, marginBottom: 4 }}>
          🧮 PARLAY CALCULATOR
        </div>
        <div style={{ fontSize: 9, color: "#444" }}>
          Enter American odds per leg — real-time payout and probability
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 9, color: "#666", letterSpacing: 2, minWidth: 60 }}>WAGER $</div>
        <input
          type="number"
          min="1"
          value={wager}
          ...
          style={{
            ...
            fontFamily: "monospace", padding: "8px 12px", borderRadius: 6,
            outline: "none", maxWidth: 160,
          }}
        />
        <div style={{ marginLeft: "auto", textAlign: "right" }}>
          <div style={{ fontSize: 8, color: "#444", letterSpacing: 1 }}>QUICK SET</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
        {legs.map((leg, i) => {
          const parsed = parseOdds(leg.odds);
          const prob = parsed !== null ? oddsToProb(parsed) : null;
          const decimal = parsed !== null ? oddsToDecimal(parsed) : null;
          const color = oddsColor(parsed);
          return (
            <div key={leg.id} style={{
              display: "flex", gap: 8, alignItems: "center",
              background: "#0a0a0a", border: `1px solid ${parsed !== null ? color + "30" : "#1a1a1a"}`,
              ...
              <div style={{
                ...
                background: parsed !== null ? color + "20" : "#111",
                color: parsed !== null ? color : "#333",
                border: `1px solid ${parsed !== null ? color + "50" : "#1a1a1a"}`,
                display: "flex", alignItems: "center", justifyContent: "center",
              ...
              <input
                ...
                placeholder="-110"
                style={{
                  width: 72, background: "#111", border: `1px solid ${parsed !== null ? color + "60" : "#2a2a2a"}`,
                  color: parsed !== null ? color : "#555",
                  fontSize: 13, fontWeight: 900, fontFamily: "monospace",
                  padding: "5px 8px", borderRadius: 5, outline: "none",
                  textAlign: "center",
                }}
              />
              ...
              {/* Implied prob */}
              {prob !== null && (
                <div style={{ fontSize: 9, color: "#444", fontFamily: "monospace", minWidth: 40, textAlign: "right" }}>
                  {(prob * 100).toFixed(1)}%
                </div>
              )}
              ...
              <button onClick={() => removeLeg(leg.id)} style={{
                ...
                color: "#2a2a2a", fontSize: 14, cursor: "pointer",
                padding: "0 4px", lineHeight: 1,
                opacity: legs.length <= 1 ? 0.2 : 1,
              }}>×</button>
            </div>
          );
        })}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <button onClick={addLeg} disabled={legs.length >= 12} style={{
          flex: 1, background: "#0a0a0a", border: "1px dashed #2a2a2a",
          color: legs.length >= 12 ? "#222" : "#444",
          fontSize: 10, fontFamily: "monospace", letterSpacing: 1,
          padding: "10px", borderRadius: 6, cursor: legs.length >= 12 ? "not-allowed" : "pointer",
        }}>+ ADD LEG {legs.length < 12 ? `(${legs.length}/12)` : "(MAX)"}</button>
      ...
      {/* Results panel */}
      <div style={{
        background: validLegs.length >= 2 ? "linear-gradient(135deg, #050505 0%, #0d1a0d 100%)" : "#050505",
        border: `2px solid ${validLegs.length >= 2 ? tierColor + "40" : "#1a1a1a"}`,
        borderRadius: 10, padding: "20px",
        boxShadow: validLegs.length >= 2 ? `0 0 28px ${tierColor}15` : "none",
      }}>
        {validLegs.length < 1 ? (
          <div style={{ textAlign: "center", color: "#222", fontSize: 10, letterSpacing: 2, padding: "20px 0" }}>
          ...
          <>
            ...
            <div style={{
              ...
              <span style={{
                fontSize: 9, letterSpacing: 2, fontWeight: 900,
                color: tierColor, background: tierColor + "15",
                border: `1px solid ${tierColor}40`,
                padding: "3px 10px", borderRadius: 3,
              }}>{tierLabel}</span>
              <span style={{ fontSize: 9, color: "#333", fontFamily: "monospace" }}>
                {validLegs.length}-leg parlay
              </span>
            </div>

            {/* Big numbers */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              <div style={{ background: "#080808", borderRadius: 8, padding: "16px 14px", textAlign: "center", border: "1px solid #111" }}>
                <div style={{ fontSize: 9, color: "#444", letterSpacing: 2, marginBottom: 6 }}>COMBINED ODDS</div>
                <div style={{ fontSize: 26, fontWeight: 900, fontFamily: "monospace", color: tierColor, lineHeight: 1 }}>
                  {validLegs.length >= 1 ? combinedOdds : "—"}
                </div>
                <div style={{ fontSize: 8, color: "#333", marginTop: 4, fontFamily: "monospace" }}>
                  {(combinedProb * 100).toFixed(2)}% implied
                </div>
              </div>
              <div style={{ background: "#080808", borderRadius: 8, padding: "16px 14px", textAlign: "center", border: "1px solid #111" }}>
                <div style={{ fontSize: 9, color: "#444", letterSpacing: 2, marginBottom: 6 }}>TOTAL PAYOUT</div>
                <div style={{ fontSize: 26, fontWeight: 900, fontFamily: "monospace", color: "#00ff87", lineHeight: 1 }}>
                  ${validLegs.length >= 1 && wagerAmt > 0 ? payout.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "—"}
                </div>
                <div style={{ fontSize: 8, color: "#333", marginTop: 4, fontFamily: "monospace" }}>
                  {wagerAmt > 0 && validLegs.length >= 1 ? `+$${profit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} profit` : "enter wager"}
                </div>
              </div>
            </div>
            ...
            <div style={{ borderTop: "1px solid #111", paddingTop: 12 }}>
              <div style={{ fontSize: 8, color: "#333", letterSpacing: 2, marginBottom: 8 }}>LEG BREAKDOWN</div>
              {validLegs.map((leg, i) => {
                const parsed = parseOdds(leg.odds);
                const prob = oddsToProb(parsed);
                const dec = oddsToDecimal(parsed);
                const color = oddsColor(parsed);
                const runningDecimal = validLegs.slice(0, i + 1).reduce((a, l) => a * oddsToDecimal(parseOdds(l.odds)), 1);
                const runningPayout = wagerAmt * runningDecimal;
                return (
                  <div key={leg.id} style={{
                    display: "flex", justifyContent: "space-between", alignItems: "center",
                    padding: "6px 0", borderBottom: i < validLegs.length - 1 ? "1px solid #0d0d0d" : "none",
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
                      <span style={{ fontSize: 9, color: color, fontFamily: "monospace", fontWeight: 700, minWidth: 20 }}>#{i + 1}</span>
                      <span style={{ fontSize: 10, color: "#aaa", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {leg.label || `Leg ${i + 1}`}
                      </span>
                    </div>
                    <div style={{ display: "flex", gap: 16, alignItems: "center", flexShrink: 0 }}>
                      <span style={{ fontSize: 11, color: color, fontWeight: 700, fontFamily: "monospace" }}>
                        {parsed >= 0 ? `+${parsed}` : parsed}
                      </span>
                      <span style={{ fontSize: 9, color: "#444", fontFamily: "monospace", minWidth: 36, textAlign: "right" }}>
                        {(prob * 100).toFixed(1)}%
                      </span>
                      {wagerAmt > 0 && (
                        <span style={{ fontSize: 9, color: "#00ff8770", fontFamily: "monospace", minWidth: 70, textAlign: "right" }}>
                          →${runningPayout.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            {/* Single bet comparison */}
            {validLegs.length >= 2 && wagerAmt > 0 && (
              <div style={{
                marginTop: 14, padding: "10px 12px",
                background: "#080808", borderRadius: 6, border: "1px solid #111",
              }}>
                <div style={{ fontSize: 8, color: "#333", letterSpacing: 2, marginBottom: 6 }}>VS SINGLE-BET COMPARISON</div>
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  {validLegs.map((leg, i) => {
                    const parsed = parseOdds(leg.odds);
                    const dec = oddsToDecimal(parsed);
                    const p = wagerAmt * dec;
                    const color = oddsColor(parsed);
                    return (
                      <div key={leg.id} style={{ fontSize: 9, color: "#444", fontFamily: "monospace" }}>
                        <span style={{ color }}>{parsed >= 0 ? `+${parsed}` : parsed}</span>
                        <span style={{ color: "#00ff8760", marginLeft: 4 }}>${p.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      {slate && !loading && (
        <div style={{ display: "flex", gap: 0, marginBottom: 18, borderBottom: "1px solid #1a1a1a" }}>
          {[
            ...
            { key: "calc",  label: "🧮 CALC",     desc: "Parlay calculator" },
          ].map(({ key, label, desc }) => (
